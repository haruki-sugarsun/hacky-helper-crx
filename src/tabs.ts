...
async function checkForDuplicates() {
    ...
    const tabsHtmlTabs = tabs.filter(tab =>
        tab.url?.includes('tabs.html') &&
        tab.id !== currentTabId
    );

    if (tabsHtmlTabs.length > 0) {
        console.log('Found another tabs.html tab in this window. Activating it and closing this one.');
...
requestTabSummariesButton.addEventListener('click', async () => {
    ...
    const tabUrls: string[] = filteredTabs.map(tab => (tab.url!)).filter(url => url);
...
function updateUI(windows: chrome.windows.Window[], tabs: chrome.tabs.Tab[]) {
    ...
    for (let i = 0; i < windows.length; i++) {
        ...
        const tabCount = state_tabs.filter(tab => tab.windowId === window.id && !tab.pinned).length;
