...
// Page Elements
const workareaContent = document.querySelector<HTMLDivElement>('#workarea_content')!
...
const interimResults = document.querySelector<HTMLSpanElement>('#interim-result')!
...
workareaContent.addEventListener('input', (_ev) => {
  ...
  chrome.storage.local.set({ 'workarea_content_save': workareaContent.innerText })
})
  ...
  recognition.onresult = function (event) {
    ...
    if (lastResult.isFinal) {
      ...
      if (transcript) {
        ...
        const textContent = monologueLog.innerText || monologueLog.textContent || '';
